...
public class SearchController extends Controller {
	 ...
	 * @return
	 */
	private static Set<Item> dbSearch(String searchTerm, boolean searchLocation) {
		if (!searchLocation) {
			 ...
			 * select distinct t0.id c0, t0.title c1, t0.description c2, t0.price c3, t0.sale_duration c4, t0.created_time c5, t0.end_time c6, t0.seller_id c7, t0.buyer_id c8, t0.address_line c9, t0.city c10, t0.state c11, t0.zipcode c12, t0.country c13, t0.contact_email c14, t0.contact_phone c15, t0.sold c16, t0.expired c17, t0.deleted c18 
				from items t0 
				join item_tags u1 on u1.item_id = t0.id 
				where t0.deleted = ? and t0.expired = ? and (lower(t0.title) like ? or lower(t0.description) like ? or u1.normalized_tag like ? or item_tags.id = ? ) ] 
			
			The ideal single query would be this
			Set<Item> itemsMatchingTitleOrDesc = Item.find.
			...
			**/
			...
			Set<Item> itemsMatchingTag = Item.find.fetch("tags").
												where().eq("deleted", false).
												eq("expired", false).
												like("tags.normalizedTag", "%" +searchTerm.toLowerCase() + "%").
												findSet();
			Set<Item> result = new HashSet<Item>(itemsMatchingTitleOrDesc);
			result.addAll(itemsMatchingTag);
			return result;
		} else {
			return Item.find.
							where().eq("deleted", false).
